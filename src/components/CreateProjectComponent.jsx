  ...
  async function submitProjectData(e) {
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        try {
          const txOptions = {
            gasLimit: gasEstimate.mul(ethers.BigNumber.from(12)).div(ethers.BigNumber.from(10)), // 20% higher gas limit
            gasPrice: gasPrice // Set gas price as per current network conditions
          };

          transaction = await contract.createNewProject(...txParams, txOptions);
          ...
          if (error.code === ethers.errors.REPLACEMENT_UNDERPRICED) {
            console.error("Transaction underpriced. Retrying with higher gas price...");
            gasPrice = gasPrice.mul(ethers.BigNumber.from(5)).div(ethers.BigNumber.from(3)); // Increase gas price
            retryCount++;
          } else {
            throw error;
          }
    ...
    <div className="create-form w-full flex items-center justify-center bg-transparent">
      <form onSubmit={submitProjectData} className="w-full max-w-2xl bg-slate-950 border-2 border-orange-400 rounded-lg border-double p-12">
        ...
        <div className="flex justify-between w-full px-8">
          <div>
            <label htmlFor="goal" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Funding Goal (Scroll)</label>
